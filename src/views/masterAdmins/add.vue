<template >
    ...
    <div class="flex flex-col justify-center items-center h-auto text-xs">
          ...
          <div class="flex text-white flex-row justify-center items-start w-full bg-blue-500 mb-10 p-4"><span class="text-4xl  font-bold">Add Admin</span></div>
          ...
          <label  class="mb-20 font-bold">Email Address</label>
                   <input list="emailList" v-model="user.mailAddress" required class=" w-full h-11 mb-4 border border-solid border-gray-200">
                        <datalist  id="emailList">
                            <template v-for="(user, userCounter) in admins" :key="userCounter">    <option :value="user.mailAddress" v-if="user.mailAddress" >{{user.mailAddress}}</option></template>
                        </datalist>
...
created(){
    this.admins = this.mainStore.getAdmins;
...
methods:{
    ...
    addUser:function(){
        ...
        var mainUser = this.admins.filter((user)=> user.mailAddress == this.user.mailAddress)[0];
        mainUser.groups = this.user.groups;
        mainUser.department = this.user.department;
        mainUser.userType = "Admin";
     
        console.log("this is the mainUser")
        console.log(mainUser)
        var data = {
            user:mainUser,
            token:this.token
        }
        console.log("this is the user being inserted");
